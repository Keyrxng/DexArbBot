...
const lookForDualTrade = async () => {
  ...
  try {
    ...
    const amountBack = await arb.calcDualTrade(
      targetRoute.router1,
      targetRoute.router2,
      targetRoute.token1,
      targetRoute.token2,
    ...
    const sizeMultiplied = tradeSize * multiplier
    const divider = ethers.BigNumber.from(10000)
    const profitTarget = sizeMultiplied / divider
...
const dualTrade = async (router1, router2, baseToken, token2, amount) => {
  ...
  try {
    ...
    const tx = await arb
      .connect(owner)
      .execDualTrade(router1, router2, baseToken, token2, amount)
    await tx.wait()
    inTrade = false
    await lookForDualTrade()
  } catch (e) {
...
const logResults = async () => {
  ...
  for (let i = 0; i < config.baseAssets.length; i++) {
    ...
    const diff =
      balances[asset.address].balance - balances[asset.address].startBalance
    const basisPoints = (diff * 10000) / balances[asset.address].startBalance
